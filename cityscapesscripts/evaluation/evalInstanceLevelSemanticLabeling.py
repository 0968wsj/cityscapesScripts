    ...
    with open(predInfoFileName, 'r') as f:
        for line in f:
            ...
            if os.path.commonprefix([filename, args.predictionPath]) != args.predictionPath:
                printError(
                    "Predicted mask {} in prediction text file {} points outside of prediction path.".format(filename, predInfoFileName))

            imageInfo = {"labelID": int(float(splittedLine[1])), "conf": float(splittedLine[2])}
...
def assignGt2Preds(gtInstancesOrig, gtImage, predInfo, args):
    ...
    for predImageFile in predInfo:
        ...
        predInstance = {
            ...
            "confidence": predConf,
            "voidIntersection": np.count_nonzero(np.logical_and(boolVoid, boolPredInst))
        }
...
def computeAverages(aps, args):
    ...
    avgDict = {"allAp": np.nanmean(aps[dInf, :, :]), "allAp50%": np.nanmean(aps[dInf, :, o50])}
...
def prepareJSONDataForResults(avgDict, aps, args):
    JSONData = {
        "averages": avgDict,
        "overlaps": args.overlaps.tolist(),
        "minRegionSizes": args.minRegionSizes.tolist(),
        "distanceThresholds": args.distanceThs.tolist(),
        "minStereoDensities": args.distanceConfs.tolist(),
        "instLabels": args.instLabels,
        "resultApMatrix": aps.tolist()
    }
